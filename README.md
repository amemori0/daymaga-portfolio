...
- **実装**：component層（`c-*`）には幅や外側marginを持たせず、幅の決定はproject層（`p-*`）側で行います。記事カード本体（`_c-card.scss`）は装飾のみで幅を持たず、列幅・カード幅はproject層のグリッドやスライダーで指定します。
  ...
  .c-card {
    --_card-padding: calc(16 * var(--to-rem));
...
- **理由**：記事カードはトップのMV・新着・人気・各一覧ページと登場箇所が多く、コピーを増やすとデザイン変更時の修正漏れが必ず発生するためです。
- **効果**：カードの修正が `card.php` 1ファイルで完結し、呼び出し側は「どのバリエーションで表示するか」だけを意識すればよくなりました。
